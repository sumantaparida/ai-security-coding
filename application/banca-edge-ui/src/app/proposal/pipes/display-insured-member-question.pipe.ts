...
export class DisplayInsuredNegativeAnswerFollowUpSection implements PipeTransform {
  ...
  transform(
    ...
    const currentMemberAnswerForQuestion = answerForCurrentQuestion.answerDetails.filter(
      (answerDetail) => answerDetail.memberId === insurerId,
    );
    ...
    if (
      value &&
      value !== '' &&
      positiveAnswerForCurrentQuestion !== value &&
      ...
      currentMemberAnswerForQuestion.forEach((answerDetail) => {
        ...
        if (positiveAnswerForCurrentQuestion !== answerDetail.answer) {
          form.addControl(
            `member-${answerDetail.memberId}-question-${question.questionId}`,
          ...
          if (question.questionType === 'YESNOFORM') {
            ...
            question.formData.forEach((formElement) => {
              ...
              form.addControl(
                `member-${answerDetail.memberId}-question-${question.questionId}-${formElement.controlName}`,
                new FormControl(answerDetail.details[formElement.controlName], validators),
              );
            });
          } else if (question.questionType === 'YESNOPOPUP') {
            if (answerDetail.details.length > 0) {
              answerDetail.details.forEach((detail, index) => {
                question.formData.forEach((formElement) => {
                  const validators = this.getValidatorsArray(formElement);
                  form.addControl(
                    `member-${answerDetail.memberId}-question-${question.questionId}-${formElement.controlName}-${index}`,
                    new FormControl(
                      answerDetail.details[index][formElement.controlName],
                      validators,
                    ),
                  );
                });
            ...
            form.addControl(
              `member-${answerDetail.memberId}-question-${question.questionId}-desc`,
              new FormControl(answerDetail.details.desc, Validators.required),
            );
          }
        } else {
          form.addControl(
          ...
          form
            .get(`member-${answerDetail.memberId}-question-${question.questionId}-desc`)
            .disable();
        }
      });
