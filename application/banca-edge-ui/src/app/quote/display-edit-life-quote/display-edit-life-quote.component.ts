...
@Component({
  selector: 'app-display-edit-life-quote',
  templateUrl: './display-edit-life-quote.component.html',
  styleUrls: ['./display-edit-life-quote.component.css'],
})
export class DisplayEditLifeQuoteComponent implements OnInit, OnChanges {
  pptOptions = {
    floor: 5,
    ceil: 50,
  };
  ...
  ngOnChanges(changes) {
    ...
    if (changes.hasOwnProperty('isDoneEditing')) {
      if (this.isDoneEditing) {
        ...
        // this.PropPaid.nativeElement.disabled = true;
        this.disabled = true;
  ...
  generateFinalQuoteData() {
    ...
    this.payoutMinValue = Math.min.apply(
      Math,
      this.finalQuote?.productQuote.map((val) => val.payoutPeriod),
    );
    this.payoutMaxValue = Math.max.apply(
      Math,
      this.finalQuote?.productQuote.map((val) => val.payoutPeriod),
    );
    ...
    console.log('printing master', this.proposerRelationMaster);
    this.relatioshipWithProposer = this.proposerRelationMaster?.find(
      (relation) => relation.id === this.finalQuote?.quoteInput?.insuredRel,
    ).value;
    console.log('printing value', this.relatioshipWithProposer);
  ...
  editQuote() {
    ...
    // this.PropPaid.nativeElement.disabled = false;
    this.disabled = false;
  }
  ...
  backrouteToQuestion() {
    ...
    this.router.navigate(['/quote/saving-traditonal', this.quoteId]);

    // this.router.navigate(['quote/saving-traditonal'], { queryParams: { quoteId: this.quoteId } });
  }
  ...
  payoutPeriodOnchange(event) {
    this.changeInQuoteInput.emit({
      minPayoutPeriodValue: event.value,
      highPayoutPeriodValue: event.highValue,
    });
  }
}
