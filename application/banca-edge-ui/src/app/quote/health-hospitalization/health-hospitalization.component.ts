...
@Component({
  selector: 'app-health-hospitalization',
  templateUrl: './health-hospitalization.component.html',
  styleUrls: ['./health-hospitalization.component.css'],
...
export class HealthHospitalizationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (
      ...
      this.healthHospForm.addControl('occupation', new FormControl('', Validators.required));
      this.healthHospForm.addControl('maritalStatus', new FormControl('', Validators.required));
      this.healthHospForm.addControl(
        'income',
        new FormControl('', [
          Validators.required,
          Validators.pattern('^[1-9][0-9]*$'),
          Validators.min(18000),
        ]),
      );
      this.healthHospForm.removeControl('policyKind');
      this.healthHospForm.removeControl('dogh');
      this.healthHospForm.removeControl('smoker');
      this.healthHospForm.removeControl('sumAssured');
      this.healthHospForm.removeControl('policyType');
      this.healthHospForm.removeControl('deductible');
      this.healthHospForm.addControl(
        'pincode',
        new FormControl('', [Validators.minLength(6), Validators.pattern(this.pattern1)]),
      );
    ...
    //   this.leadId = params['leadId'];
    //   this.customerId = params['customerId'];
    //   console.log('ininswdd query', this.customerId);
  ...
  get formError() {
    return this.healthHospForm.controls;
  }
  ...
  setFormValue() {
    ...
    if (
      ...
      this.healthHospForm.get('occupation').setValue(this.quoteInput.members[0].occupation);
      this.healthHospForm.get('maritalStatus').setValue(this.quoteInput.members[0].maritalStatus);
      this.healthHospForm.get('income').setValue(this.quoteInput?.income);
      this.healthHospForm
        .get('primaryMember')
        .setValue(this.quoteInput.members[0].relationshipId.toString());
    } else if (
      ...
      if (this.quoteInput?.deductible === 0) {
        this.healthHospForm.get('policyKind').setValue(1);
      } else {
        this.healthHospForm.get('policyKind').setValue(3);
      }
    }
    ...
    if (
      ...
      // this.healthHospForm.get('deductible').setValue(this.quoteInput?.deductible);
    }
    this.healthHospForm.get('pincode').setValue(this.quoteInput?.pincode);
    //tenure
    if (this.quoteInput?.tenure == 1) {
      this.healthHospForm.get('tenure').setValue('1');
    } else if (this.quoteInput?.tenure == 2) {
      this.healthHospForm.get('tenure').setValue('2');
    } else if (this.quoteInput?.tenure == 3) {
      this.healthHospForm.get('tenure').setValue('3');
    }
  ...
  ageCheck() {
    const timeDiff = Math.abs(
      Date.now() - new Date(this.healthHospForm.get('dob').value).getTime(),
    );
  ...
  selectPolicyType(event) {
    if (event == 1) {
      ...
      this.quoteService.getInsuredRelationship('Health', 'FF').subscribe((relationshipList) => {
        this.primaryMemberList = relationshipList;
        this.healthHospForm.get('primaryMember').setValue(this.primaryMemberList[0].id);
        this.productType = 'FF';
  ...
  generateQuote() {
    ...
    const quoteData = {
      lob: 'Health',
      leadId: this.leadId,
      // productType: this.router.url.indexOf('/quote/personal-accident') > -1 ? 'PA' : this.productType,
      // sa: this.healthHospForm.get('sumAssured').value,
      pincode: this.healthHospForm.get('pincode').value,
      tenure: this.healthHospForm.get('tenure').value,
    ...
    if (
      ...
      quoteData['income'] = this.healthHospForm.get('income').value;
      quoteData['productType'] = 'PA';
    } else {
      quoteData['sa'] = this.healthHospForm.get('sumAssured').value;
      quoteData['dogh'] = this.healthHospForm.get('dogh').value;
      quoteData['productType'] = this.productType;
    }
    if (this.productType === 'FF') {
      quoteData['members'] = this.addedMemberArr;
    } else if (this.productType === 'INDV' || this.productType === 'PA') {
      this.addedMemberArr.forEach((eachArr) => {
        if (
          this.router.url.indexOf('/quote/personal-accident') > -1 ||
          this.router.url.indexOf('/quote/pa-insurance') > -1
        ) {
          eachArr['income'] = this.healthHospForm.get('income').value;
        } else {
    ...
    if (this.isSatisfyAllCondition) {
      if (
        ...
        const primaryMemberDetails = {
          ...
          occupation: this.healthHospForm.get('occupation').value,
          maritalStatus: this.healthHospForm.get('maritalStatus').value,
          // relation:'';
        };
        if(!this.primaryAdded){
          this.addedMemberArr.push(primaryMemberDetails);
          this.primaryAdded = true;
          } 
        this.loaderService.showSpinner(true);
        ...
        quoteData['deductible'] =
          this.healthHospForm.get('policyKind').value === 3
            ? this.healthHospForm.get('deductible').value
            : 0;
        if(!this.primaryAdded){
        this.addedMemberArr.push(primaryMemberDetails);
        this.primaryAdded = true;
