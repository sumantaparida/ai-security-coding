...
@Component({
  selector: 'app-display-edit-quote',
  templateUrl: './display-edit-quote.component.html',
  styleUrls: ['./display-edit-quote.component.css'],
})
export class DisplayEditQuoteComponent implements OnInit, OnChanges {
  ...
  generateFinalQuoteData() {
    ...
    if (this.membersArr !== undefined) {
      ...
      if (this.quoteInput?.productType === 'PA') {
        this.incomeOrSa = this.finalQuote?.quoteInput?.income;
        this.editQuoteForm.get('inputOne').setValue(this.finalQuote?.quoteInput?.income);
      } else {
  ...
  routeToQuestion() {
    if (this.quoteInput?.productType === 'PA') {
      this.router.navigate(['/quote/personal-accident', this.quoteId]);
    } else {
      this.router.navigate(['/quote/health-details', this.quoteId]);
    }
  }
}
