...
@Component({
  selector: 'app-offline-motor-purchase',
  templateUrl: './offline-motor-purchase.component.html',
  styleUrls: ['./offline-motor-purchase.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class OfflineMotorPurchaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.quoteService.getOfflineProposalData(this.applicationId).subscribe(result => {
      ...
      this.insuredValue = this.offlineProposalData.applicationData.additionalData.idv;
      this.vahiType = this.offlineProposalData.applicationData.additionalData.productType;
      ...
      if (this.productType === 'Motor') {
        ...
        const regiDate = new Date(this.offlineProposalData.applicationData.additionalData.dateOfRegistration);
        const regyear = regiDate.getFullYear();
        const regmonth = ('0' + (regiDate.getMonth() + 1)).slice(-2);
        const regdt = ('0' + regiDate.getDate()).slice(-2);
        this.correctregiDate = regyear + '-' + regmonth + '-' + regdt;
        const endDate = new Date(this.offlineProposalData.applicationData.additionalData.prevPolicyEndDate);
        const endDateyear = endDate.getFullYear();
        const endDatemonth = ('0' + (endDate.getMonth() + 1)).slice(-2);
        const endDatedt = ('0' + endDate.getDate()).slice(-2);
        this.currentPolicyExpiryDate = endDateyear + '-' + endDatemonth + '-' + endDatedt;
        ...
        if (this.deductible === 0) {
          ...
          this.newPolicy = 'Renew Existing Policy';
        }
