...
@Component({
  selector: 'app-quote-annuity',
  templateUrl: './quote-annuity.component.html',
  styleUrls: ['./quote-annuity.component.css']
})

export class QuoteAnnuityComponent implements OnInit, OnDestroy {
  ...
  updateResults() {
    ...
    this.quoteService.postSubmitLifeQuote(data).subscribe(result => {
      const quote = result;
  ...
  moreDetails() {
    if (!this.isDialogOpened) {
      const dialogRef = this.dialog.open(MoreDetailAnnuityComponent, { panelClass: 'more-details-width' });
