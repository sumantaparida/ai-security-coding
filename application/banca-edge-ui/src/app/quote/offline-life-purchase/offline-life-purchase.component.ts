...
@Component({
  selector: 'app-offline-life-purchase',
  templateUrl: './offline-life-purchase.component.html',
  styleUrls: ['./offline-life-purchase.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class OfflineLifePurchaseComponent implements OnInit {
  ...
  addons;
  constructor(
    public dialog: MatDialog, private quoteService: QuoteService,
  ...
  ngOnInit(): void {
    ...
    this.offlinePurchaseForm = new FormGroup({
      ...
      insuredFirstName: new FormControl('', [Validators.required, Validators.minLength(2), Validators.maxLength(45), Validators.pattern('^[a-zA-Z ]*$')]),
      insuredLastName: new FormControl('', [Validators.required, Validators.minLength(2), Validators.maxLength(45), Validators.pattern('^[a-zA-Z ]*$')])
    });
    ...
    this.quoteService.getOfflineProposalData(this.applicationId).subscribe(result => {
      ...
      // console.log('applicationData', this.offlineProposalData);
      ...
      // console.log('test', this.additionalBenefits);
      this.addons = this.additionalBenefits?.forEach((arr) => {
        // console.log('testh', arr);
        if (arr.premium) {
      ...
      this.offlinePurchaseForm.get('insuredFirstName').setValue(this.offlineProposalData.applicationData.additionalData?.insuredFirstName);
      this.offlinePurchaseForm.get('insuredLastName').setValue(this.offlineProposalData.applicationData.additionalData?.insuredLastName);
  ...
  checkFormStatus() {
    ...
    if (this.offlineProposalData.status && this.offlineProposalData.status === 'INITIATED' || this.offlineProposalData.status === 'initiated') {
      ...
      if (this.offlineProposalData.applicationData.additionalData?.insuredRel === '1') {
        console.log('hi');
        // if(this.offlineProposalData.applicationData.proposerFirstName)
        this.offlinePurchaseForm.patchValue({
          dob: this.offlineProposalData.applicationData.additionalData?.proposerDob,
          gender: this.offlineProposalData.applicationData.additionalData?.proposerGender,
          insuredFirstName: this.offlineProposalData.applicationData.proposerFirstName,
          insuredLastName: this.offlineProposalData.applicationData.proposerLastName
        });

        this.firstName = this.offlinePurchaseForm.get('firstName') as FormControl;
        this.lastName = this.offlinePurchaseForm.get('lastName') as FormControl;

        this.firstName.valueChanges
          .subscribe((firstName) => {
            // console.log("pan", pan);
            if (firstName) {
              this.offlinePurchaseForm.get('insuredFirstName').setValue(firstName, { emitEvent: false });
            }
          });
        ...
        this.lastName.valueChanges
          .subscribe((lastName) => {
            // console.log("pan", pan);
            if (lastName) {
              this.offlinePurchaseForm.get('insuredLastName').setValue(lastName, { emitEvent: false });
            }
          });

        Object.keys(this.offlinePurchaseForm.controls).forEach(control => {
          if (control === 'dob' || control === 'gender' || control === 'insuredFirstName'
            || control === 'insuredLastName') {
  ...
  submitForm() {
    ...
    this.offlineProposalData.applicationData.additionalData.insuredFirstName = this.offlinePurchaseForm.get('insuredFirstName').value;
    this.offlineProposalData.applicationData.additionalData.insuredLastName = this.offlinePurchaseForm.get('insuredLastName').value;
  ...
  private getValidationMsg() {
    return {
      ...
      insuredFirstName: [
        ...
        { type: 'minlength', message: this.translateService.instant('error.FIRST_NAME_MAX') },
        { type: 'maxlength', message: this.translateService.instant('error.FIRST_NAME_MAX') },
        { type: 'pattern', message: this.translateService.instant('error.FIRST_NAME_INVALID') }
      ],
      insuredLastName: [
        { type: 'required', message: this.translateService.instant('error.LAST_NAME_REQUIRED') },
        { type: 'minlength', message: this.translateService.instant('error.LAST_NAME_MIN') },
        { type: 'maxlength', message: this.translateService.instant('error.LAST_NAME_MAX') },
        { type: 'pattern', message: this.translateService.instant('error.LAST_NAME_INVALID') }
      ],
    };
  }
