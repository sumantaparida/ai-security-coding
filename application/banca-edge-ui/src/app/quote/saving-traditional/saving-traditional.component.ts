...
@Component({
  selector: 'app-saving-traditional',
  templateUrl: './saving-traditional.component.html',
  styleUrls: ['./saving-traditional.component.css'],
})
export class SavingTraditionalComponent implements OnInit {
  investments = [
  ...
  ngOnInit(): void {
    this.QuoteServiceobj.getInsuredRelationship('Life', 'ENDW').subscribe((relationship) => {
    ...
    this.savingstraditionalForm.controls['proInsured'].valueChanges.subscribe((checked) => {
      if (checked === 'yes') {
        ...
        this.savingstraditionalForm.get('dateOfBirth').disable();
  ...
  submitLifeQuote() {
    ...
    if (this.savingstraditionalForm.get('startType').value === 'sa') {
      this.suminsuredValue = this.savingstraditionalForm
        ...
        .replace(/\D/g, '')
        .replace(/^0+/, '');
      lifeInsurance['sa'] = this.suminsuredValue;
    } else {
    ...
    this.QuoteServiceobj.submitLifeQuote(lifeInsurance).subscribe(
      (result) => {
        ...
        if (quote['numQuotesExpected'] > 0) {
          ...
          this.router.navigate(['/quote/quote-life-insurance', this.quoteId]);
          ...
          this.loaderService.showSpinner(false);
          this.errorDetail =
            'Oops.. We could not find any product for you. you may have to adjust your search criteria.';
