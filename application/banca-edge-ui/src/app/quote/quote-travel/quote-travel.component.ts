...
@Component({
  selector: 'app-quote-travel',
  templateUrl: './quote-travel.component.html',
  styleUrls: ['./quote-travel.component.css']
})
export class QuoteTravelComponent implements OnInit, OnDestroy {
  ...
  updateResults() {
    ...
    this.quoteService.postSubmitTravelQuote(data).subscribe(result => {
      const quote = result;
  ...
  moreDetails() {
    if (!this.isDialogOpened) {
      const dialogRef = this.dialog.open(MoreDetailsHealthComponent, { panelClass: 'more-details-width' });
