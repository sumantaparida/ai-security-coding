import { Component, OnInit } from '@angular/core';
import { MediaObserver } from '@angular/flex-layout';
import { FormBuilder, FormControl, FormGroup } from '@angular/forms';
import { MatDialog } from '@angular/material/dialog';
import { ActivatedRoute, Router } from '@angular/router';
import { LeadManagementService } from '@app/lead-management/service/lead-management.service';
import { CustomersService } from '@app/mycustomers/services/customers.service';
import { SelectInsurerModelCisComponent } from '@app/select-insurer-model-cis/select-insurer-model-cis.component';
import { usgiKyc } from '@app/shared/components/common-field-modal/field-modal-data';
import { FieldModalComponent } from '@app/shared/components/common-field-modal/field-modal/field-modal.component';
import { PolicyErrorModalComponent } from '@app/shared/components/policy-error-modal/policy-error-modal.component';
import { AccountService } from '@app/_services';
import { LoaderService } from '@app/_services/loader.service';

@Component({
  selector: 'app-cis',
  templateUrl: './cis.component.html',
  styleUrls: ['./cis.component.css'],
})
export class CisComponent implements OnInit {
  customerId: number;

  isLoading;

  quickQuoteForm: FormGroup;

  quoteData;

  isPlanCompleted;

  customerData;

  userDetails;

  productKey = [];

  orgCode;

  kblCISPlan = [{name:'Home Insurance',value:'home'},{name:'Life Insurance',value:'savings'},{name:'Motor Insurance',value:'car'}]

  insuranceProductsCopy = [];

  insuranceProducts = [
    {
      name: 'Child Education',
      score: 0,
      value: 'childEducation',
      img: '../../../assets/images/Child_Education_ICON_og.svg',
    },
    {
      name: 'Critical Illness',
      score: 0,
      value: 'ci',
      img: '../../../assets/images/Critical_illness_ICON.svg',
    },
    {
      name: 'Cyber Theft',
      score: 0,
      value: 'cyber',
      img: '../../../assets/images/anon-hacker-behind-pc.svg',
    },
    {
      name: 'Health Insurance',
      score: 0,
      value: 'health',
      img: '../../../assets/images/Health_Hospi_ICON.svg',
    },
    {
      name: 'Home Insurance',
      score: 0,
      value: 'home',
      img: '../../../assets/images/Home_Insurance_ICON.svg',
    },
    {
      name: 'Fire Insurance',
      score: 0,
      value: 'sme',
      img: '../../../assets/icons/quick-quote/standard_fire_special_perils.svg',
    },
    {
      name: 'Regular Income',
      score: 0,
      value: 'income',
      img: '../../../assets/images/Regular_Income_ICON.svg',
    },
    {
      name: 'Protection',
      score: 0,
      value: 'protection',
      img: '../../../assets/images/Protection_ICON.svg',
    },
    {
      name: 'Saving Traditional',
      score: 0,
      value: 'savings',
      img: '../../../assets/images/Savings Traditional_ICON.svg',
    },
    {
      name: 'Saving Unit Linked',
      score: 0,
      value: 'ulip',
      img: '../../../assets/images/Saving_Unit_Linked_ICON_og.svg',
    },
    {
      name: 'Personal Accident',
      score: 0,
      value: 'pa',
      img: '../../../assets/icons/quick-quote/personal_accident.svg',
    },
    {
      name: 'Motor Insurance',
      score: 0,
      value: 'car',
      img: '../../../assets/images/Car_Insurance_ICON_og.svg',
    },
    {
      name: 'Two Wheeler',
      score: 0,
      value: '2w',
      img: '../../../assets/icons/quick-quote/two_wheeler.svg',
    },
    {
      name: 'Travel',
      score: 0,
      value: 'travel',
      img: '../../../assets/icons/quick-quote/travel.svg',
    },
    {
      name: 'Offline Policies',
      score: 0,
      value: 'offline',
      img: '../../../assets/icons/quick-quote/offline_policies.svg',
    },
    {
      name: 'Shop Keeper',
      score: 0,
      value: 'sme',
      img: '../../../assets/images/Home_Insurance_ICON.svg',
    },
  ];

  sendLeadDirectlyForInsurer = [];

  mandatoryKYCReqIns = ['134']

  constructor(
    private fb: FormBuilder,
    private route: ActivatedRoute,
    public media: MediaObserver,
    private router: Router,
    private accountService: AccountService,
    private customerService: CustomersService,
    public dialog: MatDialog,
    // private spinner: NgxSpinnerService,
    private loaderService: LoaderService,
    private leadManagement: LeadManagementService,
  ) { }

  ngOnInit(): void {
   const userSubscription = this.accountService.user.subscribe(user=>{
      this.orgCode = user.organizationCode
    })
    if(this.orgCode){
      userSubscription.unsubscribe()
    }
    // this.isLoading = true;
    this.route.params.subscribe((params) => {
      if (params.customerId) {
        this.customerId = params.customerId;
        this.customerService.getCustomerByCif(this.customerId).subscribe((customerinfo) => {
          this.customerData = customerinfo;
          if (this.customerData?.planCompleted) {
            this.isPlanCompleted = true;
          } else {
            this.isPlanCompleted = false;
            // this.customerService.getQuickQuote().subscribe(() => {}); // 07/12/2021. Not sure why this is required. Will see for sometime, and remove this if no changes are done.
          }
        });
      } else {
        this.customerId = null;
      }
    });
    // this.isLoading = true;
    // this.spinner.show();
    this.loaderService.showSpinner(true);
    this.customerService.getQuickQuote().subscribe(
      (quote) => {
        // this.isLoading = false;
        this.loaderService.showSpinner(false);
        // this.spinner.hide();
        this.quoteData = quote;
        this.productKey = Object.keys(this.quoteData);

        this.insuranceProductsCopy = this.insuranceProducts.filter((arr) => {
          if (this.productKey.indexOf(arr.value) > -1) {
            return arr;
          }
        });
        if(this.orgCode === 'KB'){
          this.insuranceProductsCopy =  this.insuranceProductsCopy.filter(arr=>{
            
            if (this.kblCISPlan.findIndex(plan=>plan.value === arr.value) > -1){
              return  arr
            }
          })
          this.insuranceProductsCopy =  this.insuranceProductsCopy.map(arr=>{
            const tempVal = this.kblCISPlan.find(plan=>arr.value === plan.value)
             arr.name = tempVal.name;
             return arr
          })
        }
      },
      () => {
        this.loaderService.showSpinner(false);
        const message = 'Unable to fetch Data, Please try after sometime.';
        this.dialog.open(PolicyErrorModalComponent, {
          data: message,
          panelClass: 'dialog-width',
        });
      },
    );
    this.quickQuoteForm = new FormGroup({
      policyName: new FormControl(''),
    });
  }

  openSelectInsurer(product) {
    if (this.customerData?.planCompleted || this.orgCode === 'KB') {
      this.isPlanCompleted = true;
      const selectedProduct = this.quoteData[product.value];
      const dialogRef = this.dialog.open(SelectInsurerModelCisComponent, {
        data: {
          lob: selectedProduct.lob,
          productType: selectedProduct.productType,
        }
      });

      dialogRef.afterClosed().subscribe((insurerData) => {
        console.log(insurerData);
        if (insurerData.insurerId !== '') {
          const reqBody = {
            lob: insurerData.lob ? insurerData.lob : selectedProduct.lob,
            productType: selectedProduct.productType,
            insurerId: insurerData.insurerId,
            cifNo: this.customerId,
            channel: 'Branch',
            spCode: insurerData.spCode,
          }; if (this.mandatoryKYCReqIns.findIndex((insurer) => {
            return insurer === insurerData.insurerId;
          }) > -1) {
            // console.log('inside mandatoryKYCs')
            const customerInfo = {
              cifNumber: reqBody.cifNo,
              inputType: 1,
              orgCode: "KB",
            }
            this.customerService.getCustomerInfofromBanks(customerInfo).subscribe(customerData => {

              const dialogRef = this.dialog.open(FieldModalComponent, {
                data: { formData: usgiKyc, insurerId: reqBody.insurerId,customerData }
              })
              dialogRef.afterClosed().subscribe(data => {
                if(data!=='Cancel'){
                // console.log(data, customerData)
                //create CKYC payload
                const kycPayload = {
                  customerType: 'I',
                  customerIdNumber: data?.idNumber,
                  customerIdType: data?.idType,
                  dateOfBirth: customerData['dob'],
                  customerId: null,
                  nameAsPerAadhaar: `${customerData['firstName']} ${customerData['middleName']} ${customerData['lastName']}`,
                  gender: customerData['gender'],
                  customerEmail: customerData['email'] ? customerData['email'] : '',
                  customerMobileNo: customerData['mobileNo'],
                  customerName: `${customerData['firstName']} ${customerData['middleName']} ${customerData['lastName']}`,
                  insurerId: reqBody.insurerId,
                  customerDto: {
                    bankCustomerId: customerData['bankCustomerId'],
                    organizationCode: customerData['organizationCode'],
                    branchCode: customerData['branchCode'],
                    mobileNo: customerData['mobileNo'],
                    email: customerData['email'] ? customerData['email'] : '',
                    isIndividual: true,
                    dob: customerData['dob'],
                    firstName: customerData['firstName'],
                    middleName: customerData['middleName'],
                    lastName: customerData['lastName'],
                    gender: customerData['gender'],
                    addressList:customerData['addressList'],
                    contactList: customerData['contactList'],
                  }



                }
                // console.log('payload', kycPayload)
                //integrate kyc api endpoint.
                this.loaderService.showSpinner(true)
                this.customerService.checkCustomerKYC(kycPayload).subscribe(res => {  
                  this.loaderService.showSpinner(false)
                  if (res['isKycCompleted']==='True') {
                    this.router.navigate([
                      '/cis',
                      this.customerData.customerId,
                      reqBody.lob,
                      reqBody.productType,
                      reqBody.insurerId,
                    ]);
                  } else {
                    const param = {
                      message:'KYC link has been sent to customer for Manual KYC.',
                      link:`${res['manualKYCurl']}`,
                    }
                    const dialogRef = this.dialog.open(PolicyErrorModalComponent, {
                      data: param ,
                      panelClass: 'dialog-width',
                    });
                    dialogRef.afterClosed().subscribe(() => {
                      this.router.navigate(['/policyvault']);
                    });
                  }
                }, error => {
                  this.loaderService.showSpinner(false)
                  const dialogRef = this.dialog.open(PolicyErrorModalComponent, {
                    data: error,
                    panelClass: 'dialog-width',
                  });
                  dialogRef.afterClosed().subscribe(() => {
                    this.router.navigate(['/policyvault']);
                  });
                })
              }
              })


            })
          } else if (
            this.sendLeadDirectlyForInsurer.findIndex((insurer) => {
              return insurer === insurerData.insurerId;
            }) > -1
          ) {
            this.leadManagement.createLead(reqBody).subscribe(
              (response) => {
                this.loaderService.showSpinner(false);

                if (response['responseCode'] === 0) {
                  const message = 'Lead has been created Successfully';
                  this.dialog.open(PolicyErrorModalComponent, {
                    data: message,
                    panelClass: 'dialog-width',
                  });
                  this.router.navigate(['lead-management', 'viewleads']);
                } else if (response['responseCode'] !== 0) {
                  this.openErrorModal(response['responseMessage']);
                }
              },
              (error) => {
                this.loaderService.showSpinner(false);
                const message = 'Unable to fetch the data.';
                this.openErrorModal(message);
              },
            );
          } else {
            this.router.navigate([
              '/cis',
              'application',
              this.customerData.customerId,
              reqBody.lob,
              reqBody.productType,
              reqBody.insurerId,
            ]);
          }
        }
      });
    } else {
      this.isPlanCompleted = false;
      const dialogRef = this.dialog.open(PolicyErrorModalComponent, {
        data: 'Your Plan in not completed, Please complete the Risk Profile',
        panelClass: 'dialog-width',
      });
      dialogRef.afterClosed().subscribe(() => {
        this.router.navigate(['/needanalysis/home', this.customerData.customerId]);
      });
    }
  }

  openErrorModal(message) {
    this.dialog.open(PolicyErrorModalComponent, {
      data: message,
      panelClass: 'dialog-width',
    });
  }
}
