...
import { UpdateLeadStatusDialog } from '@app/user-management/dialog/update-lead-status.dialog';
...
export class ViewUserComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getApplicationDetails(this.reqBody);
  ...
  navigateTo(path) {
    this.router.navigate(['user-management/edit-user/', path]);
  }
  ...
  getApplicationDetails(reqBody) {
    // console.log('Application', this.reqBody)

    this.loaderService.showSpinner(true);

    this.userManService.getUsersList(reqBody).subscribe((values) => {
      this.loaderService.showSpinner(false);

      // this.policyApplicationDetailsArr = values['userDtoList'];

      values['userDtoList'].forEach((item) => {
        item['isAdmin'] = false;
        item['isUamUser'] = false;
      ...
      this.policyApplicationDetailsArr = values['userDtoList'];
      this.totalRecords = values['totalNumberOfRecords'];
  ...
  onPageChange(event: PageEvent) {
    ...
    this.userManService.getUsersList(this.reqBody).subscribe(
      (values) => {
        this.loaderService.showSpinner(false);
        this.totalRecords = values['totalNumberOfRecords'];
        values['userDtoList'].forEach((item) => {
  ...
  updateLeadStatus() {
    ...
    // });
    // dialogRef.afterClosed().subscribe((data) => {});
    const dialog = this.dialog.open(UpdateLeadStatusDialog, {
      panelClass: 'dailog-width-big',
    });
