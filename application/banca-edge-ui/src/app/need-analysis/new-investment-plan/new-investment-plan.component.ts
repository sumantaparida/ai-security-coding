...
@Component({
  selector: 'app-new-investment-plan',
  templateUrl: './new-investment-plan.component.html',
  styleUrls: ['./new-investment-plan.component.css'],
  animations: [
    trigger('newInvestmentShowHide', [
      state(
        'newInvestment',
        style({
          display: 'block',
...
export class NewInvestmentPlanComponent implements OnInit {
  @Input() state;
  @Input() form;
    ...
    if (this.investmentForm.valid) {
      ...
      if (this.investmentForm.get('Q_11_9')) {
        this.form.addControl(
          'Q_11_9_1',
          new FormControl(
            this.investmentForm.get('descrp')?.value
              ? this.investmentForm.get('descrp')?.value
              : '',
          ),
        );
  ...
  calculate(val) {
    ...
    if (this.investmentForm.valid && proceed) {
      ...
      this.userData.checkNeedAnalysis(payload).subscribe(
        (res) => {
